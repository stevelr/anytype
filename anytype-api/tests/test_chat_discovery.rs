//! Ignored live chat discovery and message workflow coverage.
//!
//! Each test uses a fresh prefix-authorized disposable space and proves whether
//! its callback ran. Run serially with explicit disposable-process admission.
...
async fn test_chat_discovery_requests() {
    let callback_ran = Arc::new(AtomicBool::new(false));
    let callback_flag = callback_ran.clone();
    let outcome = Box::pin(with_disposable_space_context(
        "chat-discovery-resolver",
        move |ctx| {
    ...
    .await
    .expect("cleanup-safe chat-discovery live harness");
    assert_disposable_completed(outcome, &callback_ran, "chat-discovery live suite");
}

#[tokio::test]
#[ignore = "requires configured real server and disposable test admission"]
#[serial_test::serial(disposable_anytype_api)]
async fn test_chat_message_resolution_requests() {
    let callback_ran = Arc::new(AtomicBool::new(false));
    let callback_flag = callback_ran.clone();
    let outcome = Box::pin(with_disposable_space_context(
        "chat-discovery-message-resolver",
        move |ctx| {
            callback_flag.store(true, Ordering::SeqCst);
            Box::pin(async move {
                ...
                .map_err(|_| TestError::Assertion {
                    message:
                        "unknown message order-id resolution exceeded its fixed live-test timeout"
                            .to_owned(),
                })?;
    ...
    .await
    .expect("cleanup-safe chat-message resolver live harness");
    assert_disposable_completed(outcome, &callback_ran, "chat-message resolver live suite");
}
