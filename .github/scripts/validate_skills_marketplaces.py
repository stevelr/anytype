...
def load_object(path: pathlib.Path, context: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise MarketplaceValidationError(f"{context}: expected UTF-8 text ({error})") from error
...
def sole_plugin(catalog: dict[str, Any], context: str) -> dict[str, Any]:
    ...
    require(isinstance(plugins, list) and len(plugins) == 1, f"{context}: expected exactly one plugin")
    plugin = plugins[0]
    require(isinstance(plugin, dict), f"{context}: plugin entry must be an object")
    require(plugin.get("name") == PLUGIN_NAME, f"{context}: plugin name must be {PLUGIN_NAME!r}")
    return plugin
...
def resolve_source(root: pathlib.Path, source: Any, context: str) -> pathlib.Path:
    ...
    require(isinstance(source, str) and source.startswith("./"), f"{context}: source must start with ./")
    require("\\" not in source and "\x00" not in source, f"{context}: source uses unsafe separators")
    relative = pathlib.PurePosixPath(source.removeprefix("./"))
    require(relative.parts and ".." not in relative.parts, f"{context}: source escapes the repository")
    resolved = root.joinpath(*relative.parts).resolve()
    try:
        resolved.relative_to(root)
    except ValueError as error:
        raise MarketplaceValidationError(f"{context}: source escapes the repository") from error
    require(resolved == root / PLUGIN_PATH, f"{context}: source must resolve to ./skills")
    require(resolved.is_dir() and not resolved.is_symlink(), f"{context}: plugin directory is missing or unsafe")
    return resolved
...
def validate_catalogs(root: pathlib.Path) -> None:
    ...
    codex_manifest = load_object(root / PLUGIN_PATH / ".codex-plugin/plugin.json", "Codex plugin manifest")
    claude_manifest = load_object(root / PLUGIN_PATH / ".claude-plugin/plugin.json", "Claude plugin manifest")
    ...
    require(codex_entry.get("category") == "Productivity", "Codex marketplace category is inconsistent")

    claude_entry = sole_plugin(claude, "Claude marketplace")
    resolve_source(root, claude_entry.get("source"), "Claude marketplace")
    require(claude_entry.get("strict") is True, "Claude marketplace must use strict plugin loading")
    for field in SHARED_FIELDS:
        require(
            claude_entry.get(field) == claude_manifest.get(field),
            f"Claude marketplace {field} does not match the plugin manifest",
        )
    require(codex_manifest.get("name") == PLUGIN_NAME, "Codex plugin manifest name is inconsistent")
    require(
    ...
    require(claude.get("owner") == expected_owner, "Claude marketplace owner does not match plugin author")
    metadata = claude.get("metadata")
    require(isinstance(metadata, dict), "Claude marketplace metadata must be an object")
    require(metadata.get("version") == claude_manifest.get("version"), "Claude marketplace version does not match plugin manifest")
