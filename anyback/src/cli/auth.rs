...
use crate::cli::{AppContext, AuthArgs, AuthCommands, emit_json};

pub async fn handle(ctx: &AppContext, args: AuthArgs) -> Result<()> {
    match args.command {
        AuthCommands::Login { force } => login(ctx, force).await,
        AuthCommands::Logout => logout(ctx),
        AuthCommands::Status => status(ctx).await,
        AuthCommands::SetHttp => set_http(ctx),
        AuthCommands::SetGrpc {
            config,
            account_key,
            token,
...
async fn login(ctx: &AppContext, force: bool) -> Result<()> {
    ...
    emit_json(&json!({ "authenticated": true }))
}

fn logout(ctx: &AppContext) -> Result<()> {
    ctx.client.logout()?;
    emit_json(&json!({ "authenticated": false }))
}
...
fn set_http(ctx: &AppContext) -> Result<()> {
    ...
    emit_json(&serde_json::json!({ "http_credentials": "updated" }))
}
...
fn set_grpc(
    ...
    emit_json(&serde_json::json!({ "grpc_credentials": "updated" }))
}
