...
impl ArtifactStaging {
    ...
    pub(crate) async fn consume(&self, source: &mut StageSource) -> Result<(), StagingError> {
        ...
        *source.lease = RecordState::Consumed { source: retained };
        Ok(())
    }
...
fn transition_to_cleanup_pending(state: &mut RecordState) -> bool {
    ...
    let (destination, source, published_without_source) = match prior {
        ...
        RecordState::Ready { source, .. }
        | RecordState::Available { source, .. }
        | RecordState::Consumed { source, .. } => (None, Some(source), false),
...
mod tests {
    ...
    async fn expiry_removes_indeterminate_publication_and_its_quota_record() {
        ...
        let mut destination = lease.take_destination().expect("export destination");
        destination.write_all(b"hello").expect("write export");
        let source = destination.commit_retained().expect("publish export");
        drop(source);
