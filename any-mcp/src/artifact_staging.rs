            ...
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    builder
                        ...
                        // A client that shuts down its write side after a
                        // complete request still receives the response; the
                        // header timeout and operation deadlines keep the
                        // connection bounded.
                        .half_close(true)
...
mod tests {
    ...
    fn staging_test_config(root: &Path) -> ArtifactConfig {
        staging_test_config_with_limits(root, "")
    }

    fn staging_test_config_with_limits(root: &Path, limits_toml: &str) -> ArtifactConfig {
